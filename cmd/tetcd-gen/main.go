...
func main() {
	...
	statements, _ := buildStructs(root, "", "github.com/NHAS/tetcd/paths", "github.com/NHAS/tetcd/codecs")
...
func buildNode(name string, t types.Type, compress bool) (*node, error) {
	...
	for i := range strct.NumFields() {
		...
		// If we're an explicitly ignored field, or if we're an unexported field
		if slices.Contains(tagParts, "-") ||
			(!field.Exported() && !field.Anonymous()) {

			result.ignored[field.Name()] = true
			continue
		}
...
func buildStructs(root *node, currentPath, pathsPkg, codecsPkg string) ([]jen.Code, []string) {
	var (
		result       []jen.Code
	...
	for _, name := range sortedKeys(root.children) {
		...
		statements, ignored := buildStructs(child, path.Join(currentPath, root.name), pathsPkg, codecsPkg)
	...
	if currentPath == "" {
		...
		initBody := buildInitBody(root, ignoredPaths)
...
func buildInitBody(root *node, ignoredPaths []string) []jen.Code {
	var stmts []jen.Code
	leaves := collectAllLeaves(root, "")
	for _, lp := range leaves {
		...
		stmts = append(stmts,
			jen.Id(root.name+"Differ").Dot("Register").Call(chain),
		)
	}

	lits := []jen.Code{}
	for _, path := range ignoredPaths {
		lits = append(lits, jen.Lit(path))
	}

	stmts = append(stmts,
		jen.Id(root.name+"Differ").Dot("Ignore").Call(lits...),
	)
	return stmts
}
